...
impl Pdu for ResupplyReceivedPdu {
        ...
        Self: Sized,
    {
        ...
        if header.pdu_type != PduType::ResupplyReceived {
            return Err(DISError::invalid_header(
                format!(
                    "Expected PDU type ResupplyReceived, got {:?}",
                    header.pdu_type
                ),
                None,
            ));
...
impl ResupplyReceivedPdu {
    ...
    /// # Examples
    ///
    /// Initializing an `ResupplyReceivedPdu`:
    /// ```
    /// use open_dis_rust::logistics::ResupplyReceivedPdu;
    /// let pdu = ResupplyReceivedPdu::new();
    /// ```
    ///
    pub fn new() -> Self {
        let mut pdu = Self::default();
        pdu.pdu_header.pdu_type = PduType::ResupplyReceived;
