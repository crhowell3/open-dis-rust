...
impl Pdu for SetDataReliablePdu {
    fn length(&self) -> u16 {
        let length = PduHeader::LENGTH + EntityId::LENGTH * 2 + 1 + 1 + 2 + 4 + 4 + 4;
...
impl SetDataReliablePdu {
    ...
    /// # Examples
    ///
    /// Initializing an `SetDataReliablePdu`:
    /// ```
    /// use open_dis_rust::simulation_management_with_reliability::SetDataReliablePdu;
    /// let pdu = SetDataReliablePdu::new();
    /// ```
    ///
    pub fn new() -> Self {
        let mut pdu = Self::default();
        pdu.pdu_header.pdu_type = PduType::SetDataReliable;
...
mod tests {
    ...
    fn serialize_then_deserialize() {
        ...
        let new_pdu = SetDataReliablePdu::deserialize(&mut deserialize_buf).unwrap();
