...
impl Pdu for ActionResponseReliablePdu {
        ...
        Self: Sized,
    {
        ...
        if header.pdu_type != PduType::ActionResponseReliable {
            return Err(DISError::invalid_header(
                format!(
                    "Expected PDU type ActionResponseReliable, got {:?}",
                    header.pdu_type
                ),
                None,
            ));
...
impl ActionResponseReliablePdu {
    /// Creates a new `ActionResponseReliablePdu`
    ///
    /// # Examples
    ///
    /// Initializing an `ActionResponseReliablePdu`:
    /// ```
    /// use open_dis_rust::simulation_management_with_reliability::ActionResponseReliablePdu;
    /// let pdu = ActionResponseReliablePdu::new();
    /// ```
    ///
    pub fn new() -> Self {
        let mut pdu = Self::default();
        pdu.pdu_header.pdu_type = PduType::ActionResponseReliable;
...
mod tests {
    ...
    fn deserialize_header() {
        ...
        let new_pdu = ActionResponseReliablePdu::deserialize(&mut deserialize_buf).unwrap();
